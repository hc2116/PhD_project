...
##### Mahoney, 2003
##### Florida Institute of Technology, 300 citations, 5 pages

* Only on first few packets of server requests
* Flag events that have not been observed for a long time
* model the first 48 bytes of the packet starting with the IP header
* anomaly score on each of the 48 instances $\sum tn/r$ (r=256, the number of possible bytes) 
...
## Traffic aggregation/time-window based approaches

### Wavelet/Volume-based

#### A Signal Analysis of Network Traffic Anomalies
##### Barford et al., 2002
##### Wisconsin-Madison, 1060 citations, 12 pages

Approach: 
* Collect aggregated volume traffic of flows (sampled with rate 1/96 though) at different spots in network
* Model the volume of the traffic with wavelets at different frequency domains, detect even short volume anomalies like flash crowds reliably

My criticism: 
* Although fairly sophisticated and detailed approach (I did not understand everything), for us there is no need (at least anymore) to look at traffic aggregated over many machines
  --> Loss of a lot of detail
* Only suitable to unstealthy attacks that create big volume anomalies

#### A transform domain-based anomaly detection approach
##### D.Jian et al., 2014
##### Schanghai, 120 citations, 15 pages

* Takes into account network topology
* S-transform based (kind of wavelet/fourier transform):
* Extract only hf domain of OD flows to discover anomalies (which are comparibly small, but fast and bursty) 

* Steps: 
 * For a network with $q$ nodes, group all origin–destination flows in it into $q$ different groups $f_{1,...,q}+\{f_{1q},f_{2q},...\}$, which contains all the OD flows running through the particular node
 * For $f_{1,...,q}$, perform S-transform and extract high frequency component (since bursts correspond to HF domain)
 * Carry out inverse S-transform and reconstruct signal only using HF components
 * Compute average pairwise correlation coeff. between OD flows in each group using sliding window
 * If average correlation is to small (below treshold), an anomaly is found


#### Anomaly detection in IP networks
##### Thottan and Ji, 2003
##### Bell Lab and Georgia Tech, 600 citations, 14 pages

Similar to Barford et al., detecting abrupt changes in traffic volume of SNMP collector (similar to network flows I suppose):
* Cannot directly observe multivariate volume changes in different channels (different collectors)
  * Therefore, the abrupt changes in each of the MIB variables is first obtained
* Abrupt changes are detected by comparing the variance of the residuals obtained from two adjacent windows of data that are referred to as the learning and test windows
* Change detection is done using a hypothesis test based on the generalized likelihood rati
...
##### Dewaele et al., 2007
##### Lyon/Tokyo, 150 citations, 8 pages

* Aimed at both short-lived and long-lasting low-intensity anomalies

* Sliding window $T$
  * Observe $I$ packets in $T$ with usual 5-touple (t, SIP, DIP, SPort, DPort)
  * Use $N=8$ independent tabluar-hash functions (no idea how that works) with $M=32$ being the size of has table to randomly project SIP,DIP,SPort,DPort onto "sketch"
  * Apparently, marginal distributions of aggregated traffic (probably of the parameters projected by the hash functions) can be well described using gamma distributions, whose parameters scale with aggregation window size
  * Estimate gamma parameters to optain behaviour profile, and then find statistical distances using mahalanobis distance 
  * Anomaly detections are performed in all $M$ outputs of the $N$ hash functions, reversing the hash gives indication of the anomalous parameter
  
My comment: 
* Very complicated, why should the deviations of the marginal distributions correspond to proper traffic anomalies? If I understood correctly, they only translate IPs and Ports, which can change without corresponding to anomalies
* Why only look at marginal distributions?
* Temporal variations?
...
Is then extended by Camacho et al. to  do automatic PCA anomaly detection (which has already been around, but not used by Lakhina)
...
+ the false positive rate is very sensitive to small differences in the number of principal components in the normal subspace
...
+ Note for me: There is far less UDP than TCP traffic in internet, but UDP flows are less aggregated, there are similar numbers of UDP and TCP flows


#### Mining Anomalies using traffic feature distributions
##### Anukool Lakhina, Mark Crovella, Christophe Diot, 2005
##### Boston University, 1330 citations, 12 pages

* Measure network-wide sample entropy of feature histogram:
$H(X)=-\sum_{i=1}^N\frac{n_i}{S}\log\big(\frac{n_i}{S}\big)$
where $S=\sum_i^N n_i$
![Design](Mining_anomalies.png){ width=70% }

 * Write $H$ as three-way matrix according to time, feature, and OD flow (not very clear what exactly that means)
 * PCA-like subspace projection
 * Clustering to identify anomalies
 * Tested on Abilene and Geant backbone network using untruthed data
...
##### University of New Brunswick, 160 citations, 16 pages

* Aggregate traffic in 1 minute intervals:
 * #TCP flows
 * #UDP flows
 * #ICMP flows
 * Average #packets and #bytes per flow in each protocol
 * ratio of packets to bytes per flow in each protocol

* Fit wavelet model to traffic features

* Residuals of model as outliers
  * Model residuals of normal traffic with GMM
    * A GMM can approximate any distribution when sufficient number of components are used   
...
##### Cordero et. al, 2016
##### TU Darmstadt, 8 pages, 5 citations

* Using lakhina's approach of binning flows into time windows 
  * calculating the entropy of each feature inside this time window (count the occurrences of individual IP addresses, etc.)
 * Input for replicator network (specific type of  auto-encoder)
  * dropout in hidden layers 2 and 4 to prevent overfitting

* Remove outliers during training by looking at the quantiles of the reconstruction error, some anomalies are quite obvious
  * Claim to be resilient against learning in the presence of attacks

* Tested on MAWI Japan-USA dataset with injected synthetic attacks
* In the evaluation, it is only able to detect very broad port scans and large DDoS attacks, (i.e. with larger volume, more concise traffic like smaller scans or less attacks are not detected)

## Flow or packet-metadata based approaches
### PCA-based

#### Identifying Intrusions in Computer Networks with Principal Component Analysis
##### Wang and Battiti, 2006
##### University of Trento, Italy, 110 citations, 8 pages

PCA directly on the 34 numeric features of Darpa 1998 data

If reconstruction error from the first few components compared to true projection is too high --> Anomaly

#### Service Specific Anomaly Detection for Network Intrusion Detection 
##### Kruegel et. al, 2002,
##### TU Vienna, 420 citations, 8 pages

Approach: 
* "partition the network traffic and independently analyze packets sent by different applications"
  * "By concentrating only on one type of traffic (hence the term service specific), statistical data with lesser variance can be collected. This allows to establish a notion of 'normal traffic' for each service" (Me liky)
  * HTTP, DNS, SMTP, and FTP cover most of the necessary services
  * Current model only trained on DNS and HTTP traffic

In other words: Train separate models on each port with the same approach

Model:
  * Packet Processing Unit: Read packet and extract service request (such as get, head r post for htt)
  * Statistical Processing Unit: Give each event an anomaly score composed of three parts:
  $AS=0.3\cdot AS_{service-type}+0.3\cdot AS_{length}+0.4\cdot AS_{payload}$
    * Type of service: Compute frequencies $p_t$ of each type of service and score $AS_{type}=-\log_2(p_t)$ so that less frequent services get higher score
    * Length: Calculate mean $mu_s$ and std $sigma_s$ for each service 
      * "the length of different requests does not vary for different requests, but is usually a short string in human readable form"
      * $AS_{len}=1.5^\frac{l-\mu_s}{2.5\cdot sigma_t}$
    * Payload distribution: Based on the assumption of human readable string (i.e. non-uniform distribution of strings)
      * Calculate frequency of characters, build probability distribution as average of all payload
      * $\Chi^2$ test to measure deviation of payload from distribution 
      
My comment: Is likely to work well for applications that only send short and isolated service requests like dns (also since it emphasizes length and content of payload), unlikely to work well on general traffic like ssh, ftp etc.

#### A multi-model approach to the detection of web-based attacks
##### Kruegel et. al, 2005
##### UC Santa Barbara, 290 citations, 22 pages

Similar approach as Kruegel developed before, but now only directed towards server receiving http requests:

Score anomaly of individual requests is composition of multiple models:
$\text{AS}=\sum_{m\in \text{models}}w_m \cdot (1-p_m)$

1. Attribute length:
  * Only if $l$ is greater than $\mu$, $p_m(l)$=\frac{\sigma^2}{(l-\mu)^2}
2. Attribute character dstribution: Like above
3. Structural inference on parameters (i.e. if characters have been replaced by different ones, non-printable characters, etc. for buffer overflow, directory travesal,...)
  * Use of markov model
4. Token finder
5. Attribute presence or absence
6. Attribute order
7. Access frequency
  * Calculate probability of number of requests from client $n_1$ conditional on total number of requests $n$ in an intervla
8. Time delays between individual requests
  * Dstribution of time delays is compiled on the fly to adapt to changes, then $\chi^2$ test 
9. Invocation order

My comment: Highly specific, and does not relate individual attribute anomalies with each other (i.e. how does it adapt to changes in time and temporal variation that is correlated in multiple attributes?)

#### Learning Nonstationary Models of Normal Network Traffic for Detecting Novel Attacks
##### Mahoney and Chan, 2002
##### Florida Institute of Technology, 390 citations, 10 pages

Two models working parallel: 
* PHAD (packet header anomaly detection)
  * Looking at individual packets and giving them an anomaly score
  * 33 fields from Ethernet, IP, and transport layer
  * Treating all of them as continuous (ok....), and allowing $C$ number of clusters in each of them
  * Each new packet creates a cluster in each field on its own, but if $C$ is exceed, the closest clusters are merged together (they tested different $C$-values, from 32 to 1000)
  * Anomaly score is the number $r$ of fields in which the number of clusters is updated 
* ALAD (Application layer anomaly detection)
  * Looking at connections and asign them a score
  * Tested attributes:
  $P(src IP | dest IP)$
  $P(src IP | dest IP, dest port)$
  $P(dest Ip, dest Port)$
  $P(TCP flags | dest port)$
  $P(keyword | dest port)$, keyword is the first word on a line of input (I assume for unencrypted traffic)

* total anomaly score is $t \cdot r\n$ where $t$ is the time of last occurrence
...
* Kayacik (2007) et. al refine it more for KDD99 data, Jirapummin et al. also use it first (2002), but not well described, they detect from visualization
	* Luo et al. (2014) also detect from visualisation using 4-angle-star technique to distinguish 5 classes (sounds stupid)
...
* Bayesian Network classification on Darpa 

#### Intrusion Detection based on K-Means Clustering and Naïve Bayes Classification
##### Muda et. al, 2011
##### Malaysia, 104 citations, 6 pages

On KDD Cup 99 data
Clustering to create attack clusters, then classifier to classify these different attack types
Altwaijry et al (2012) also used a Naive Bayese on KDD

#### CANN: An intrusion detection system based on combining cluster centers and nearest neighbors
##### Lin et. al, 2016
##### Taiwan, 170 citations, 9 pages

Combination of Clustering and NN on KDD Cup 99

Firstly, the cluster centres are extracted using a 
clustering technique. As the second step, the distance measures 
are calculated, namely the distance between all data of the given 
dataset and the cluster centres (dist1) and the distance between 
each data point and its nearest neighbour in the same cluster 
(dist2). The sum of dist1 and dist2 leads to a new one dimension 
feature. Finally, k-NN is used to classify the data represented 
by the newly constructed feature. Experiments show that the 
approach succeeds in detecting DoS and probe attacks, but does 
not perform well on U2R and R2L


#### Principle Components Analysis and Support Vector Machine based Intrusion Detection System
##### Eid et al., 2010
##### Cairo, 70 citations, 5 pages

PCA on KDD features, then SVM


#### Online Adaboost-Based parametrized methods for dynamic distributed network instrusion detection
##### Hu et. al, 2014
##### Bejing, 100 citations, 17 pages

New: Online parameter estimation and pooling of distributed models on each node into global model, also boosting was not used before but does not look like a step forward

But: classification based

Steps:
 * Weak classifiers for each individual feature of KDD 99, both categorical and continuous
 * Online adaboost with decision stumps (old) or with online GMMs (new, but why GMMs as classifiers?)
 * One model for each node, then combining them into global model using PSO and SVM-based algorithm
  * Two approaches for combining models exist:
    * One is rule based, uses product, sum, max, min, median and majority vote rule
    * Other is SVM based by combining model outputs of all classifiers into a vector and then using and SVM to classify the result using all model recommendations
  * Both are combined to account for performance gaps and weight differences for similar models

#### An Adaptive Automatically Tuning Intrusion Detection System
##### Yu and Tsai, 2008
##### University of Illinois, 70 citations, 25 pages

Very interesting approach, albeit quite different from regular machine learning

* Uses fuzzy set/control theory to build fuzzy classifiers
  * These aren't exact, but also quantify some sort of confidence in their prediction
  * Due to inexactness, it is easier to tune the borders of the classifiers
  * They use an approach earlier developed by Yu (the other) to tune the borders of the classifier in an online manner by giving different weights to correctly classified instances, false positives, and false negatives

* The different weighting of regular data, and false positives or false negatives (how do you find false negatives when operating) might be important and worth considering for us


* Classifier is ultimately a group of binary classifiers working on KDD 99 data
  * One binary classifier for each malware group
...
A good amount of rather badly written papers (from mostly chinese or korean Universities, many with a good number of citations) exist that apply different deep learning architectures to intrusion detection, but focus very heavily on explaining the particular architecture and the training method while giving extremely little detail about the specific way they are applied to intrusion detection. Often, the only information is that the particular architecture is applied to the KDD-Cup dataset.
...
#### A Deep Learning Approach for Network Intrusion Detection Systems
##### Zhang, Li, Meo, Manikopoulos, Jorgenson, 2001
##### New Jersey, 240 citations, 6 pages

* KDD-Cup dataset
* Simply feature extraction using auto-encoders, and then supervised classification using soft-max regression on the extracted features  (trained separately)
* Training using 10-fold cross validation (ok....)

#### An Intrusion Detection Model Based on Deep Belief Networks 
##### Gao, Gao, Gao, Wang, 2014
##### Northwest University Xi'an, 46 citations, 6 pages

Training of Deep Belief Network (stacked RBM) to the KDD-Cup data
  * Categorical variables have to be pre-processed to be numerical
  * Reconstruction error is indicator of anomaly


### Semantics/Temporal correlation based

Several approaches of discrete Markov chain models or Hidden Markov Models, but only for system call logs, such as Lane (1999, 2006), Yeung and Ding (2--3), orHu et al. (2009).

#### Real-Time Dynamic Network Anomaly Detection 
##### Noble and Adams, 2018
##### Imperial College, 0 citations, 14 pages

* "correlation is a more complicated statistic, it has the added advantag e that it may be more resistant to poisoning, and provides a greater challenge for a sophisticated attacker to conceal their malicious activity"

* monitor the correlation between event (netflow) inter-arrival times and either byte or packet size on a single edge
* Monitor the two features as bivariate gaussian rv with drifting mean and covariance matrix
* Specific online estimation method of covariance (and thus pearson correlation $\rho$) between the two to allow for expected temporal variation

* Anomaly detection as change in correlation structure
  * This change is detected by comparing the static and the online estimate of $\rho$ using hypothesis test
    * if no change in correlation structure, static converges faster to true $\rho$
    * if change, online estimate converges faster
* Anomaly for p-value $\alpha$, scoring across $M$ edges has false-alert probability $\alpha^M$

Overall assumption: Different edges are independent, if changes happen simultaneously, there is malicious activity

My comment: Why should the change of the correlation be an indicator of malicious activity


### Fingerprinting and traffic classification
...
##### Sun et al., 2002
##### Stanford, 360 citations, 12 pages

* We show that it suffices to identify a signicant fraction of ecrypted webpages quite reliably using number and size of unordered set of objects (blocks of packets) fetched by browser
* 200 target pages
* Similarity measure assumptions
  * $t\in T$ set of pages
  * Identifiable page as $t'$ if $\forall u\neq t: Sim(t',t)>Sim(t',u)$ and $Sim(t',t)>\geq c$
  * Similarity measure is Jaccard coefficient (not properly explained how that works for the described features)

Counter measures:
* Padding (needs lot of padding, expensive)
* Mimicking of other websites (difficult to achieve)
* Pre-fetching 
* Pipelining
...
##### Wang et. al, 2017
##### Hefei/Bejing University, 17 citations, 6 pages

Look at the first 700 or so bytes of connection, apply CNN for supervised classification of traffic
...
## Detection via System calls, binaries, ...
