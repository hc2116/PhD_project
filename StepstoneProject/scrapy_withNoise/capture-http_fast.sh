...
[ -z "$PROJECT_NAME" ] && PROJECT_NAME="scrapy_stepping_stone_fast"
...
do
    ...
    timeout -k $(($DURATION2+3)) $DURATION2 docker exec -ti $(sudo docker ps -aqf "name=${PROJECT_NAME}_spider_1") scrapy runspider "scrapy/spider_${PROJECT_NAME}.py"; EXIT_CODE=$?
    END=$(date +%s)
    DIFF=$(( $END - $START ))
