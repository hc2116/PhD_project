...
Several examples of pivoting can be found in the Operation Aurora \cite{tankard2011advanced}, in the Operation Night Dragon \cite{mcafee2014report}, in the Black Energy \cite{eisac2017report} malware that in December 2015 compromised the Ukrainian powergrid, in the MEDJACK \cite{ayala2016active} attack in 2016 where attackers stole health data by using medical devices as stepping-stones. The infamous Archimedes tool \cite{archimedes2017} used by the CIA leverages stepping-stone chains to reach the LAN of target hosts.

All these examples demonstrate that stepping-stones are an important and challenging research problem. 
