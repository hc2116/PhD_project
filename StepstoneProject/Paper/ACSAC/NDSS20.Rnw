...
\begin{figure}
\centering
\includegraphics[width=0.75\textwidth]{images/Stepstone_final_2.png}
...
To create a fair playing field for the selected SSD methods, we only look at connections that exchange more than 1500 packets and exclude shorter connections from both the compiled background dataset and the attack dataset. 
...
Table \ref{Tab:Summary} contains a summary of the included methods. We labelled each method to make referring to it in the evaluation easier.
