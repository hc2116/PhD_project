...
<<Noevasion_4nodes, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3,fig.width=8,fig.cap="ROC-curves for different SSD methods on dataset A (no evasive tactics). Anomaly-based methods are excluded.",cache=TRUE>>=
require(ggplot2)
require(xtable)
