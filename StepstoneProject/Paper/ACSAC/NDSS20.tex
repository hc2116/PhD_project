...
\textcolor{black}{Several examples of the use of stepping-stone chains can be found in the Operation Aurora \cite{tankard2011advanced}, in the Operation Night Dragon \cite{mcafee2015report}, in the Black Energy \cite{eisac2017report} malware that in December 2015 compromised the Ukrainian powergrid, in the MEDJACK \cite{ayala2016active} attack in 2016 where attackers stole health data by using medical devices as stepping-stones.} The infamous Archimedes tool \cite{archimedes2017} used by the CIA leverages stepping-stone chains to reach the LAN of target hosts. The recent emergence of connected IoT-devices increases the potential attack surface as printers, cameras, or even thermostates can been successfully used as stepping stones \textcolor{red}{insert citation from Infosec Resources, 2019}.
...
\item Our evaluation shows that while most methods can accurately detect command propagation in different settings, detection rates vanish for all methods when enough chaff is inserted. This result disproves the \textcolor{red}{claims} made for multiple methods that detection rates are robust against chaff perturbations.
