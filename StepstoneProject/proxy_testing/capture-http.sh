...
#export SSH_SERVER_IP=localhost
export SSH_SERVER_IP="129.215.228.22"
...
do
    ...
    docker exec -ti $(sudo docker ps -aqf "name=${PROJECT_NAME}_spider_1")  scrapy runspider 'scrapy/spider.py'
    sleep 1
    teardown
#    echo $EXIT_CODE
...
#    else
#    	echo ${PROJECT_NAME}_relay_stepstone-${CAPTURETIME}-sc${SCENARIO}-$REPNUM "," $NETCAT_PARA1 "," $NETCAT_PARA2 "," $NETCAT_PARA3 "," $NETCAT_PARA4 "," $NETCAT_PARA5 "," $NETCAT_PARA6 "," $DELAY_PARA1 "," $DELAY_PARA2 "," $DELAY_PARA3 "," $DELAY_PARA4 "," $DELAY_PARA5 "," $DELAY_PARA6 >> ${PROJECT_NAME}.txt
#    fi
